...
/**
 * An enemy that looks like a slime.
 * 
 * @author Evan Brown
 * @version 1.0
 */
public class Slime extends Character implements EnemyAI
{
    int tempX; 
    int tempY;
        ...
        else
        {
            if(hp <= 0)
            {
                ...
                for(int i = 0; i < MyWorld.worldObj.length; i++)
                {
                    ...
                    {
                        if(MyWorld.worldObj[i][x] instanceof Slime)
                        {
                            Slime e = (Slime)MyWorld.worldObj[i][x];
                            if(e.getHp() <= 0)
                            {
