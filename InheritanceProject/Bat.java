...
/**
 * An enemy that looks like a bat.
 * 
 * @author Evan Brown
 * @version 1.0
 */
public class Bat extends Character implements EnemyAI
{
    int tempX; 
    int tempY;
     ...
     * Constructor
     */
    public Bat(int hp, int atk, int def)
    {
        this.hp = hp;
        this.atk = atk;
        this.def = def;
        ...
        else
        {
            if(hp <= 0)
            {
                ...
                for(int i = 0; i < MyWorld.worldObj.length; i++)
                {
                    ...
                    {
                        if(MyWorld.worldObj[i][x] instanceof Bat)
                        {
                            Bat e = (Bat)MyWorld.worldObj[i][x];
                            if(e.getHp() <= 0)
                            {
