    ...
    public void battleMove()
    {
        if(hp > 0)
        {
            ...
            if(MyWorld.party.size() == 2)
            {
                ...
                {
                    if(Math.abs(getY() - temp.getY()) > 4)// && !isTouching(Obstacle.class))
                    {
                        if(getY() < temp.getY())
                        {
                            setImage("images\\EvilFront.png");
        ...
        else
        {
            if(hp <= 0)
            {
                ...
                for(int i = 0; i < MyWorld.worldObj.length; i++)
                {
                    ...
                    {
                        if(MyWorld.worldObj[i][x] instanceof Enemy)
                        {
                            Enemy e = (Enemy)MyWorld.worldObj[i][x];
                            if(e.getHp() <= 0)
                            {
