...
/**
 * An enemy that looks like a final boss.
 * 
 * @author Evan Brown
 * @version 1.0
 */
public class Boss extends Character implements EnemyAI
{
    int tempX; 
    int tempY;
     ...
     * Constructor
     */
    public Boss(int hp, int atk, int def)
    {
        this.hp = hp;
        this.atk = atk;
        this.def = def;
        ...
        else
        {
            if(hp <= 0)
            {
                ...
                for(int i = 0; i < MyWorld.worldObj.length; i++)
                {
                    ...
                    {
                        if(MyWorld.worldObj[i][x] instanceof Boss)
                        {
                            Boss e = (Boss)MyWorld.worldObj[i][x];
                            if(e.getHp() <= 0)
                            {
